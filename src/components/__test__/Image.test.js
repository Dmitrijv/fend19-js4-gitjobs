...
describe("Test Image component for expected behaviour", () => {
  const jobItem = {
  ...
  let wrapper;
  beforeEach(() => {
    wrapper = mount(<Image imgUrl={jobItem.company_logo} />);
  });

  it("should be contained in the correct wrapper", () => {
    expect(wrapper.find("div.image-container img").length).toBe(1);
  });

  it("should have the correct image class", () => {
    expect(wrapper.find("img.company-logo").length).toBe(1);
  });

  it("should display the correct image", () => {
    expect(wrapper.find("img.company-logo").props().src).toBe(jobItem.company_logo);
  });
});
